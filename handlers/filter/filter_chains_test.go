...
func (tc filterChainTestCase) Test(t *testing.T) {
	...
	core.AssertEqual(t, tc.expectedCount, len(msgs), "message count")
...
func (tc nestedFilterTestCase) Test(t *testing.T) {
	...
	msgs := base.GetMessages()
	if tc.shouldPass {
		core.AssertEqual(t, 1, len(msgs), "message should pass")
		if len(msgs) > 0 {
			core.AssertEqual(t, tc.testLevel, msgs[0].Level, "level preserved")
		}
	} else {
		core.AssertEqual(t, 0, len(msgs), "message should be blocked")
	}
}
...
func (tc dynamicThresholdTestCase) Test(t *testing.T) {
	...
	msgs := base.GetMessages()
	core.AssertEqual(t, 1, len(msgs), "only error should pass initially")
	base.Clear()
	...
	msgs = base.GetMessages()
	core.AssertEqual(t, 2, len(msgs), "info and error should pass")
	base.Clear()
	...
	msgs = base.GetMessages()
	core.AssertEqual(t, 2, len(msgs), "all should pass")
}
...
func runTestCompleteTransformationChain(t *testing.T) {
	...
	msgs := base.GetMessages()
	core.AssertEqual(t, 2, len(msgs), "should have 2 messages (3rd dropped)")

	// Verify first message transformations
	msg1 := msgs[0]
	core.AssertEqual(t, "[1] message 1", msg1.Message, "message transformed")
	core.AssertEqual(t, "value", msg1.Fields["normal"], "normal field")
	core.AssertNil(t, msg1.Fields["drop_this"], "dropped field absent")
	core.AssertEqual(t, "value", msg1.Fields["renamed_me"], "field renamed")
	core.AssertEqual(t, "[REDACTED]", msg1.Fields["redact"], "field redacted")

	// Verify second message transformations
	msg2 := msgs[1]
	core.AssertEqual(t, "[2] message 2", msg2.Message, "message transformed")
	core.AssertEqual(t, true, msg2.Fields["fields_processed"], "fields filter applied")
...
func runTestFilterNilAndEmptyHandling(t *testing.T) {
	...
	msgs := base.GetMessages()
	core.AssertEqual(t, 3, len(msgs), "all messages logged")

	// Check nil field
	msg1 := msgs[0]
	core.AssertNil(t, msg1.Fields["nil_value"], "nil value preserved")

	// Check empty message
	msg2 := msgs[1]
	core.AssertEqual(t, "", msg2.Message, "empty message preserved")

	// Check empty key ignored
	msg3 := msgs[2]
	core.AssertEqual(t, "included", msg3.Fields["valid"], "valid field present")
