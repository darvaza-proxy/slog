...
func runTestEmptyFieldKeyIgnored(t *testing.T) {
	...
	core.AssertEqual(t, "included", msgs[0].Fields["valid"], "valid field")
	core.AssertNil(t, msgs[0].Fields[""], "empty key not present")
}
...
func runTestStackWithVariousSkipValues(t *testing.T) {
	...
	msgs := base.GetMessages()
	core.AssertEqual(t, 2, len(msgs), "message count")
...
func runTestRecursiveFilterScenarios(t *testing.T) {
	...
	msgs := base.GetMessages()
	core.AssertEqual(t, 1, len(msgs), "message reaches base")
	core.AssertEqual(t, "nested message", msgs[0].Message, "message content")
	core.AssertEqual(t, "value", msgs[0].Fields["nested"], "field value")
}
