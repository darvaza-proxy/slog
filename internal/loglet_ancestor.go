...
// 3. AND either:
//   - pathToRoot has length 1 (current loglet already has cache)
...
func (info logletAncestorInfo) canDelegate() bool {
	...
	// Can delegate if this loglet itself has the cache, or if no fields
	// need to be added beyond the cached ancestor
	return len(info.pathToRoot) == 1 || info.totalFields == len(info.baseMap)
}
