...
Handler implementations should embed `Loglet` and delegate field management:
...
func (h *Handler) WithField(key string, value any) slog.Logger {
    return &Handler{
        Loglet:  h.Loglet.WithField(key, value),
        backend: h.backend,
    }
}
...
func (h *Handler) print(msg string) {
    ...
    if fields := h.FieldsMap(); fields != nil {
        h.backend.LogWithFields(h.Level(), msg, fields)
    } else {
        h.backend.Log(h.Level(), msg)
    }
}
```
